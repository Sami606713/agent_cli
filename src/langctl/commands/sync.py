...
def sync(
    ...
    if path.is_file():
        try:
            ...
            console.print("{WARN} langgraph.json is not valid JSON — regenerating")
    ...
    if missing or extra:
        console.print("{WARN} pyproject.toml dependencies are out of date:")
    ...
    if drift and not force:
        console.print("{WARN} langgraph.json differs from agent.yaml:")
    ...
    if check:
        if missing or extra:
            ...
            raise typer.Exit(1)
        console.print("{CHECK} langgraph.json and pyproject.toml are in sync")
        return
    ...
    if sync_dependencies(project.spec, pyproject):
        console.print("{CHECK} wrote pyproject.toml (dependencies)")
